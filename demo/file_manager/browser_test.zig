...
fn appendBrowserTestEntries(state: *State, count: usize) !void {
    for (0..count) |index| {
        try state.model.entries.append(allocator, .{
...
fn initBrowserListTestState(state: *State, ctx: *goop.Context, text_measure_ctx: *const goop.TextMeasureCtx) !void {
    state.model.current_dir = try allocator.dupe(u8, "/tmp");
    state.runtime.text_measure_ctx = text_measure_ctx;
    state.runtime.ctx = ctx;
...
test "file browser can sort directories with the active field" {
    ...
    state.model.sort_directories_together = true;
...
test "file browser detail wrapper inserts line breaks for long names" {
    ...
    state.runtime.ui_scale = 2;
    state.runtime.logical_width = 800;
    state.runtime.text_measure_ctx = &text_measure_ctx;
...
test "file browser selected name text click starts inline rename" {
    ...
    state.runtime.logical_width = 960;
    state.runtime.logical_height = 720;
    state.model.current_dir = try allocator.dupe(u8, "/tmp");
    state.runtime.text_measure_ctx = &text_measure_ctx;
    state.runtime.ctx = &ctx;
    try state.model.entries.append(allocator, .{
    ...
    try appendSelectedPathIfMissing(&state, state.model.entries.items[0].path);
    ...
    const name_rect = entryNameTextRect(&state, &ctx, 0, state.model.entries.items[0]).?;
    try std.testing.expect(pointHitsEntryNameText(&state, &ctx, 0, state.model.entries.items[0], name_rect.x + 1, name_rect.y + name_rect.h * 0.5));
    try beginRenameEntry(&state, &ctx, state.model.entries.items[0]);
    try buildWidgetTree(&state);
    ctx.doLayout(&text_measure_ctx);

    const input = state.interaction.rename_input_handle orelse return error.TestUnexpectedResult;
...
test "file browser blank asset space can clear selection" {
    ...
    const scroll_rect = ctx.tree.getConst(state.view.chrome.file_panel_scroll.?).layout_rect;
    const row_rect = ctx.tree.getConst(state.view.assets.row_handles.items[0]).layout_rect;
    const blank_x = scroll_rect.x + 24;
    const blank_y = row_rect.y + row_rect.h + 24;

    const body_rect = ctx.tree.getConst(state.view.assets.asset_table_body.?).layout_rect;
    ...
    try std.testing.expectEqual(@as(usize, 0), state.model.selected_paths.items.len);
}
...
test "file browser blank asset space starts marquee before any selection" {
    ...
    try std.testing.expect(ctx.tree.getConst(state.view.assets.asset_table_body.?).kind.table.internal.marquee_active);
    try std.testing.expect(ctx.tree.getConst(state.view.assets.row_handles.items[0]).kind.table_row.selected);
}
...
test "file browser list refresh grows blank marquee hit area with viewport" {
    ...
    state.runtime.logical_width = 960;
    state.runtime.logical_height = 360;
    state.model.current_dir = try allocator.dupe(u8, "/tmp");
    state.runtime.text_measure_ctx = &text_measure_ctx;
    state.runtime.ctx = &ctx;
    try state.model.entries.append(allocator, .{
...
test "file browser syncs toolkit selection from visible entry window" {
    ...
    state.view.assets.asset_visible_start = 3;
    ...
    try std.testing.expectEqual(@as(usize, 1), state.model.selected_paths.items.len);
    try std.testing.expectEqualStrings("/tmp/entry-004.txt", state.model.selected_paths.items[0]);
...
test "file browser selection detail does not resize the list at ui scale 2" {
    ...
    const width_before = ctx.tree.getConst(state.view.assets.asset_table.?).layout_rect.w;
    try appendSelectedPathIfMissing(&state, state.model.entries.items[0].path);
    try setSelectedPath(&state, state.model.entries.items[0].path);
    try buildWidgetTree(&state);
    ctx.doLayout(&text_measure_ctx);
    ...
    const width_after = ctx.tree.getConst(state.view.assets.asset_table.?).layout_rect.w;
...
test "folder tree derives partial ancestors and expanded current directory" {
    var state = State{};
    state.model.current_dir = try allocator.dupe(u8, "/home/user/project");
    defer {
        allocator.free(state.model.current_dir);
...
test "folder tree preserves previous current branch when navigating to ancestor" {
    var state = State{};
    state.model.current_dir = try allocator.dupe(u8, "/home/user/project/goop");
    defer {
        allocator.free(state.model.current_dir);
        clearTrackedPaths(&state.view.assets.folder_tree_expanded_paths);
        state.view.assets.folder_tree_expanded_paths.deinit(allocator);
...
test "sidebar scroll survives widget tree rebuild" {
    ...
    try buildWidgetTree(&state);
    ctx.doLayout(&text_measure_ctx);
    const first_scroll = state.view.chrome.sidebar_scroll.?;
    ...
    try buildWidgetTree(&state);
    ctx.doLayout(&text_measure_ctx);
    const second_scroll = state.view.chrome.sidebar_scroll.?;
...
test "browser list window keeps rendered body aligned with logical row offset" {
    var state = State{};
    defer state.model.entries.deinit(allocator);
    ...
    state.model.file_panel_scroll_y = 104;
    ...
    const expected_range = browserVirtualRange(state.model.entries.items.len, visible_start, visible_count);
    ...
    const total_height = row_height * @as(f32, @floatFromInt(state.model.entries.items.len));
...
test "browser grid window keeps rendered body aligned with logical row offset" {
    ...
    state.runtime.logical_width = 800;
    state.model.file_panel_scroll_y = 640;
    ...
    const total_rows = std.math.divCeil(usize, state.model.entries.items.len, window.columns) catch unreachable;
...
test "file browser list scroll keeps existing window until the render range changes" {
    ...
    const scroll_handle = state.view.chrome.file_panel_scroll.?;
    const row_height = browserListRowHeight(&state);
    const visible_count = browserVisibleCount(state.view.layout.file_panel_viewport_height, row_height);
    ...
    const start_before = state.view.assets.asset_visible_start;
    const end_before = state.view.assets.asset_visible_end;
    ...
    try std.testing.expectEqual(start_before, state.view.assets.asset_visible_start);
    try std.testing.expectEqual(end_before, state.view.assets.asset_visible_end);
...
test "file browser list preserves row continuity across virtualization boundaries" {
    ...
    const first_row = state.view.assets.row_handles.items[logical_entry_index - state.view.assets.asset_visible_start];
    ...
    const rebuilt = try syncBrowserTestScrollFrame(&state, &ctx, &text_measure_ctx);
    const second_row = state.view.assets.row_handles.items[logical_entry_index - state.view.assets.asset_visible_start];
    const second_y = ctx.tree.getConst(second_row).layout_rect.y;

    try std.testing.expect(rebuilt);
    try std.testing.expectEqual(chunk_rows - browser_overscan_rows, state.view.assets.asset_visible_start);
...
test "file browser list covers the viewport after a large scroll jump" {
    ...
    try std.testing.expect(rebuilt);
    try std.testing.expect(state.view.assets.asset_visible_start <= target_row);
    try std.testing.expect(state.view.assets.asset_visible_end >= target_row + visible_count);
}
